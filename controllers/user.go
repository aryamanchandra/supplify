...
import (
	...
	helpers "github.com/aryamanchandra/supplify/helpers"
...
func Signup() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			log.Panic(err)
			c.JSON(http.StatusInternalServerError, gin.H{"error": "error occured while checking through email"})
		}
...
func GetUser() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err := helpers.UserVerify(c, userId); err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
			return
		}
		var ctx, cancel = context.WithTimeout(context.Background(), 100*time.Second)
